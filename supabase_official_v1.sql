-- 牙套时间管家 V1.0 正式版数据库 + Storage 升级 SQL
-- 使用方法：Supabase → SQL Editor → New Query → 粘贴整段 → Run
-- 可重复执行，不会删除已有数据。
...
drop trigger if exists set_aligner_records_updated_at on public.aligner_records;
create trigger set_aligner_records_updated_at before update on public.aligner_records
for each row execute function public.set_updated_at();
...
with check (bucket_id = 'ortho-photos' and auth.uid()::text = (storage.foldername(name))[1]);

create policy "ortho_photos_update_own"
on storage.objects for update to authenticated
using (bucket_id = 'ortho-photos' and auth.uid()::text = (storage.foldername(name))[1])
with check (bucket_id = 'ortho-photos' and auth.uid()::text = (storage.foldername(name))[1]);
